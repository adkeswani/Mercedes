...
void main() {
  ...
  group('ProgramRepository', () {
    ...
    test('publishVersion creates version and updates header', () async {
      ...
      expect(versionDoc.entries.length, 2);
    ...
    test('publishVersion preserves workoutName', () async {
      ...
      final versionDoc = await repo.getVersion(id, 1);
      expect(versionDoc!.entries[0].workoutName, 'Push Day');
      expect(versionDoc.entries[1].workoutName, 'Pull Day');
    });
    ...
    test('publishVersion handles null workoutName', () async {
      ...
      final versionDoc = await repo.getVersion(id, 1);
      expect(versionDoc!.entries[0].workoutName, isNull);
    });
    ...
    test('getLatestEntries returns workouts from latest version',
      ...
      final refs = await repo.getLatestEntries(id);
    ...
    test('create stores folderId as null by default', () async {
      final id = await repo.create(
        name: 'Loose Program',
        type: ProgramType.assignable,
        userId: 'coach1',
    ...
    test('setFolder moves program into a folder and back out', () async {
      final id = await repo.create(
        name: 'Foldered',
        type: ProgramType.assignable,
        userId: 'coach1',
    ...
    test('setFolder throws when caller is not the owner', () async {
      final id = await repo.create(
        name: 'Foldered',
        type: ProgramType.assignable,
        userId: 'coach1',
    ...
    test('publishVersion round-trips dayOffset', () async {
      ...
      final version = await repo.getVersion(id, 1);
      expect(version!.entries[0].dayOffset, 0);
