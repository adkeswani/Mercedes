...
if ($Environment -eq 'prod' -and -not $AllowProduction) {
    throw 'Production canary seeding requires -AllowProduction.'
}
...
$nodeScript = Join-Path $PSScriptRoot 'release\seed-release-canary.js'
$adminModule = Join-Path $repoRoot 'functions\node_modules\firebase-admin'
if (-not (Test-Path -LiteralPath $nodeScript -PathType Leaf)) {
    throw "Missing release canary seed implementation: $nodeScript"
}
...
if ($LASTEXITCODE -ne 0) {
    throw 'Release canary seeding failed.'
}
