...
- Athlete Exercise Notes (per-athlete, per-exercise-template personal notes; displayed wherever that exercise appears in the athlete's workouts; scoped to a specific exercise template object so notes don't cross trainers)
...
3. 🔶 [MVP] Auth + athlete assignment: sign-in, role model, enrollment, athlete exercise notes, athlete personal program creation (with copy + nesting + day assignment parity), and schedule assignment to athletes. — *Phase 3A done (enrollment system, roster management, ownership checks, program type toggle). 292 tests. Remaining: workout scheduling, exercise notes, security rules.*
4. ✅ [MVP] Trainer program organization + scheduling calendar: program folders for owners, reusable program schedule templates (relative-day/recurrence schedule materialized into dated instances at assignment), and a per-athlete trainer calendar spanning all of the owner's programs (color-coded by program) with click-to-edit/reschedule and add-workout-to-a-day. — *Completed 2026-06-21. Program folders, relative-day program schedules materialized at assignment, per-athlete color-coded trainer calendar with reschedule/edit/add, unified assignment flow, and flattened roster with per-program removal. 395 tests. Deployed to web.*
...
6. ⬚ [MVP] Load/difficulty model: RPE + duration logging, workout-type-based weighting, dashboard load summaries, and athlete download/export of workout + load data.
7. ⬚ [Post-MVP] Community features (forum): program-level forum with reply notifications and athlete download/export of community conversation data.
8. ⬚ [Post-MVP] Marketplace and access lifecycle: program discovery, durations, consent/waiver tracking, expiry notifications, automatic enrollment removal, and paid Program Owner entitlement for assignable-program creation.
9. ⬚ [Post-MVP] Group workouts/plans: shared group progress, completion visibility, and group comments visible to members of the same group.
...
4. Trainer program organization + scheduling calendar is complete when owners can group programs into folders, save reusable program schedules expressed relative to a start date (recurrence/day-offsets) that materialize into dated workout instances at assignment, and use a per-athlete calendar across all their programs (color-coded by program) to view, reschedule/edit, and add workouts on specific days — with owner edits limited to scheduling fields and completion remaining athlete-only.
...
6. Load/difficulty model is complete when required RPE + duration are captured on completion, load points are computed client-side with versioning and server-side audit capability, dashboard widgets return correct weekly/type/bucket summaries, and athletes can export/download their workout + load history.
7. Community features (forum) are complete when program-level forum threads/replies and reply notifications are live with correct per-program visibility, and athletes can export/download their community conversation data.
8. Marketplace and access lifecycle is complete when program discovery is usable, duration-based access rules are enforced, consent/waiver status is tracked, pre-expiry notifications are sent, removals write `removedAt`, and paid Program Owner entitlement gates assignable-program creation.
9. Group workouts/plans is complete when owners can run shared group plans, members can see group completion status appropriately, and group comments are visible to other members of the same group with role/privacy boundaries enforced.
...
- [MVP] Exercise notes: athletes can add personal notes to any exercise template (e.g. equipment settings, form cues) that appear wherever that exercise is used in any of their workouts, including workouts from other program owners; notes are private to the athlete and scoped to the specific exercise template object so they don't cross between different trainers' exercises.
...
- [MVP] Exercise notes are per-athlete, per-exercise-template. They are stored as a subcollection under the user document and keyed by exercise template ID. Notes are private to the athlete and appear in any workout containing that exercise — including workouts created by other program owners. Notes don't transfer when a different trainer creates an exercise with the same name (different exercise template ID).
...
- [MVP] Add/view exercise notes:
	1. Athlete views a workout containing an exercise.
	2. Athlete taps on the exercise to view or add a personal note (e.g. "bar height = 3", "use blue band").
	3. Note is saved and appears wherever that exercise template is used in the athlete's workouts.
	4. Athlete can edit or delete the note at any time.
